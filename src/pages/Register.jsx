...
const api = {
  register: async (payload) => {
    ...
    // return data;
    if (payload.email === "test@test.com") throw new Error("Cet email est déjà utilisé.");
    return { success: true, token: "mock_jwt_token_abc" };
  }
};
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // TODO: localStorage.setItem('authToken', data.token);
      setAlert({ msg: "✅ Compte créé avec succès ! Redirection...", type: "success" });
  ...
  return (
    <div className="min-h-screen bg-[#0B0F1A] text-[#E8EDF5] font-ubuntu flex relative overflow-hidden">
      {/* Background */}
      ...
      <div className="hidden lg:flex flex-col w-[42%] shrink-0 border-r border-white/[0.06] p-10 relative z-10">
        ...
        <div className="flex-1 flex flex-col justify-center">
          <h2 className="font-unbounded font-black text-3xl leading-tight tracking-tight mb-4">
            Rejoignez +2 000<br />équipes{" "}
            <span className="text-[#3590E3]">qui vendent</span><br />
            <span className="text-[#BAF09D]">plus vite.</span>
          </h2>
          <p className="text-sm text-white/45 leading-relaxed mb-8">
            Commencez gratuitement. Aucune carte bancaire. Annulation libre.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <Input
              ...
              id="email"
              type="email"
              placeholder="kofi@entreprise.com"
              value={form.email}
              onChange={set("email")}
              error={errors.email}
              autoComplete="email"
