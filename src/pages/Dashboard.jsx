...
import {
  IoChatbubblesOutline, IoTicketOutline, IoFlashOutline, IoBarChartOutline,
  IoPeopleOutline, IoSettingsOutline, IoSearchOutline, IoNotificationsOutline,
  IoSendOutline, IoChevronBackOutline, IoChevronForwardOutline, IoEllipsisHorizontal,
  IoCloseOutline, IoCheckmarkCircleOutline, IoTimeOutline, IoAlertCircleOutline,
  IoAddOutline, IoTrashOutline, IoPencilOutline, IoAttachOutline, IoChatbubbleOutline,
} from "react-icons/io5";
import {
  HiOutlineFire, HiOutlineTrendingUp, HiOutlineCurrencyDollar, HiOutlineClipboardList,
  HiOutlineMail, HiOutlineCalendar, HiOutlineRefresh, HiOutlineChip, HiOutlineCreditCard,
} from "react-icons/hi";
...
const INITIAL_TICKETS = [
  { id: "T-041", title: "Problème d'intégration KKiaPay", description: "Le webhook KKiaPay ne se déclenche pas après paiement. Erreur 500.", category: "incident", priority: "high", status: "open", assignedTo: "Équipe Tech", client: "Sophie Martin", createdAt: "2024-01-15", dueDate: "2024-01-17", comments: [{ author: "Agent IA", text: "Ticket créé automatiquement.", time: "5 min" }], attachments: [] },
  { id: "T-040", title: "Export CSV des contacts", description: "L'export CSV ne fonctionne pas pour les listes de plus de 1000 contacts.", category: "support", priority: "medium", status: "pending", assignedTo: "David Lambert", client: "David Koné", createdAt: "2024-01-15", dueDate: "2024-01-20", comments: [], attachments: [] },
  { id: "T-039", title: "Webhook ne se déclenche pas", description: "Le webhook configuré pour les événements Slack ne se déclenche pas.", category: "incident", priority: "high", status: "open", assignedTo: "Équipe Tech", client: "Amina Traoré", createdAt: "2024-01-14", dueDate: "2024-01-16", comments: [], attachments: [] },
  { id: "T-038", title: "Question sur la facturation", description: "Différence entre plans Pro et Enterprise.", category: "commercial", priority: "low", status: "resolved", assignedTo: "Marie Dupont", client: "Romain Dubois", createdAt: "2024-01-13", dueDate: "2024-01-14", comments: [{ author: "Marie Dupont", text: "Réponse envoyée par email.", time: "1 j" }], attachments: [] },
];
...
const MOCK_CHAT_HISTORY = {
  1: [
    ...
    { role: "ai",   text: "Parfait ! Avec 25 personnes, notre plan Pro à 29 900 FCFA/mois est idéal. Il inclut 5 canaux, WhatsApp inclus et 5000 messages/mois. Voulez-vous plus de détails ?", time: "10:03" },
...
const AI_RESPONSES = [
  "Excellente question ! La mise en place prend généralement 48h. Notre équipe vous accompagne pas à pas. Souhaitez-vous planifier un onboarding ?",
  "Notre plan Starter à 9 900 FCFA/mois convient aux petites équipes. Le Pro à 29 900 FCFA/mois est notre plus populaire avec WhatsApp inclus !",
  "Notre essai gratuit de 14 jours vous donne accès à toutes les fonctionnalités. Aucune carte requise. Voulez-vous créer votre compte ?",
  "PulsAI s'intègre avec Zapier, Slack, HubSpot et 200+ outils. L'intégration prend moins de 5 minutes.",
  "Pour 25 personnes, le plan Enterprise à 99 900 FCFA/mois est recommandé. Canaux illimités, IA personnalisée et support dédié 24/7.",
];
...
const KPIS = [
  ...
  { label: "Revenus ce mois",       value: "124 700 FCFA",  delta: "+18%", Icon: HiOutlineCurrencyDollar, color: "purple" },
];
...
const NOTIFS = [
  ...
  { Icon: MdOutlinePayment,   iconColor: "text-[#BAF09D]",  text: "Paiement KKiaPay reçu — Amina 29 900 FCFA",    time: "1 h"   },
...
const QUICK_ACTIONS = [
  { Icon: HiOutlineClipboardList, label: "Créer un ticket", action: "tickets" },
  { Icon: HiOutlineMail,          label: "Séquence email",  action: null      },
  { Icon: HiOutlineCreditCard,    label: "Lien paiement",   action: "payment" },
  { Icon: HiOutlineCalendar,      label: "Planifier appel", action: null      },
];
...
function ScoreBadge({ score }) {
  const cls = score >= 85 ? "text-[#BAF09D] bg-[#BAF09D]/10 border-[#BAF09D]/25" : score >= 65 ? "text-yellow-300 bg-yellow-300/10 border-yellow-300/25" : "text-white/40 bg-white/5 border-white/10";
...
function Avatar({ initials, size = "sm", gradient = false }) {
  ...
  return <div className={`${sz} rounded-full flex items-center justify-center font-unbounded font-bold shrink-0 ${gradient ? "bg-gradient-to-br from-[#3590E3] to-[#BAF09D] text-[#0B0F1A]" : "bg-[#3590E3]/20 text-[#3590E3] border border-[#3590E3]/30"}`}>{initials}</div>;
}
...
function FunnelBar({ label, count, max, color }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 h-2 bg-white/[0.06] rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${w}%`, background: color }} /></div>
...
function TicketRow({ ticket, onClick, onEdit, onDelete }) {
  const priority = { high: "bg-red-400/10 text-red-400 border-red-400/25", medium: "bg-yellow-400/10 text-yellow-400 border-yellow-400/25", low: "bg-white/5 text-white/35 border-white/10", urgent: "bg-red-600/20 text-red-300 border-red-600/30" }[ticket.priority] || "bg-white/5 text-white/35 border-white/10";
  const statusCfg = { open: { cls: "bg-[#3590E3]/10 text-[#3590E3]", label: "Ouvert", Icon: IoAlertCircleOutline }, pending: { cls: "bg-yellow-400/10 text-yellow-400", label: "En attente", Icon: IoTimeOutline }, "in-progress": { cls: "bg-purple-400/10 text-purple-400", label: "En cours", Icon: IoTimeOutline }, resolved: { cls: "bg-[#BAF09D]/10 text-[#BAF09D]", label: "Résolu", Icon: IoCheckmarkCircleOutline }, closed: { cls: "bg-white/5 text-white/30", label: "Fermé", Icon: IoCheckmarkCircleOutline } }[ticket.status] || { cls: "bg-white/5 text-white/30", label: ticket.status, Icon: IoAlertCircleOutline };
  return (
    <div onClick={() => onClick && onClick(ticket)} className="flex items-center gap-4 py-3 border-b border-white/[0.05] last:border-0 hover:bg-white/[0.02] px-2 -mx-2 rounded-xl transition-colors cursor-pointer group">
      <span className="font-mono text-xs text-white/25 w-12 shrink-0">{ticket.id}</span>
      <div className="flex-1 min-w-0"><p className="text-xs font-medium text-white/80 truncate group-hover:text-white transition-colors">{ticket.title}</p><p className="text-[0.65rem] text-white/30 mt-0.5">{ticket.client} · {ticket.assignedTo}</p></div>
      <span className={`text-[0.6rem] font-semibold px-2 py-0.5 rounded-full border hidden sm:inline ${priority}`}>{ticket.priority}</span>
      <span className={`flex items-center gap-1 text-[0.65rem] font-medium px-2 py-1 rounded-lg ${statusCfg.cls}`}><statusCfg.Icon size={11} />{statusCfg.label}</span>
      {(onEdit || onDelete) && (
...
function TypingIndicator() {
  return (
    <div className="flex items-end gap-2 mb-3">
      <div className="w-7 h-7 rounded-full bg-[#3590E3]/20 border border-[#3590E3]/30 flex items-center justify-center shrink-0"><TbRobot size={14} className="text-[#3590E3]" /></div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,15,26,0.85)", backdropFilter: "blur(8px)" }}>
      <div className="w-full max-w-2xl bg-[#111827] border border-white/[0.09] rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06] shrink-0"><div className="flex items-center gap-2"><IoTicketOutline size={16} className="text-[#3590E3]" /><h2 className="font-unbounded font-semibold text-sm text-white">{isEdit ? `Modifier ${form.id}` : "Nouveau ticket"}</h2></div><button onClick={onClose} className="w-7 h-7 rounded-lg bg-white/[0.05] hover:bg-white/[0.1] flex items-center justify-center text-white/30 hover:text-white transition-all"><IoCloseOutline size={16} /></button></div>
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          <div><label className={labelCls}>Titre *</label><input className={inputCls} placeholder="Ex: Problème d'intégration…" value={form.title} onChange={e => set("title", e.target.value)} /></div>
          <div><label className={labelCls}>Description</label><textarea className={`${inputCls} resize-none`} rows={4} value={form.description} onChange={e => set("description", e.target.value)} /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className={labelCls}>Catégorie</label><select className={`${inputCls} cursor-pointer`} value={form.category} onChange={e => set("category", e.target.value)}>{["support","incident","demande interne","commercial"].map(c => <option key={c} value={c} style={{ background: "#111827" }}>{c.charAt(0).toUpperCase()+c.slice(1)}</option>)}</select></div>
            <div><label className={labelCls}>Priorité</label><select className={`${inputCls} cursor-pointer`} value={priorityVal[form.priority]||form.priority} onChange={e => set("priority", priorityKey[e.target.value]||e.target.value)}>{["basse","moyenne","haute","urgente"].map(p => <option key={p} value={p} style={{ background: "#111827" }}>{p.charAt(0).toUpperCase()+p.slice(1)}</option>)}</select></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className={labelCls}>Statut</label><select className={`${inputCls} cursor-pointer`} value={statusVal[form.status]||form.status} onChange={e => set("status", statusKey[e.target.value]||e.target.value)}>{["ouvert","en cours","résolu","fermé"].map(s => <option key={s} value={s} style={{ background: "#111827" }}>{s.charAt(0).toUpperCase()+s.slice(1)}</option>)}</select></div>
            <div><label className={labelCls}>Assigné à</label><input className={inputCls} value={form.assignedTo} onChange={e => set("assignedTo", e.target.value)} /></div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div><label className={labelCls}>Client</label><input className={inputCls} value={form.client} onChange={e => set("client", e.target.value)} /></div>
            <div><label className={labelCls}>Créé le</label><input type="date" className={inputCls} value={form.createdAt} onChange={e => set("createdAt", e.target.value)} /></div>
            <div><label className={labelCls}>Échéance</label><input type="date" className={inputCls} value={form.dueDate} onChange={e => set("dueDate", e.target.value)} /></div>
          </div>
          <div>
            <label className={labelCls}>Commentaires</label>
            {form.comments.map((c, i) => (<div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06] mb-2"><div className="w-6 h-6 rounded-full bg-[#3590E3]/20 border border-[#3590E3]/30 flex items-center justify-center shrink-0 text-[0.55rem] font-unbounded font-bold text-[#3590E3]">{c.author.split(" ").map(w=>w[0]).join("").slice(0,2)}</div><div><div className="flex items-center gap-2 mb-0.5"><span className="text-[0.65rem] font-medium text-white/60">{c.author}</span><span className="text-[0.6rem] text-white/20">{c.time}</span></div><p className="text-xs text-white/50">{c.text}</p></div></div>))}
            <div className="flex items-end gap-2"><textarea className={`${inputCls} flex-1 resize-none`} rows={2} placeholder="Ajouter un commentaire…" value={newComment} onChange={e => setNewComment(e.target.value)} /><button onClick={addComment} disabled={!newComment.trim()} className="w-9 h-9 rounded-xl bg-[#3590E3]/20 hover:bg-[#3590E3]/30 text-[#3590E3] flex items-center justify-center transition-all disabled:opacity-30 shrink-0"><IoChatbubbleOutline size={14} /></button></div>
          </div>
        </div>
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/[0.06] shrink-0">
          <button onClick={onClose} className="text-xs px-4 py-2 rounded-xl border border-white/[0.08] text-white/40 hover:text-white/70 transition-all">Annuler</button>
          <button onClick={() => onSave(form)} disabled={!form.title.trim()} className="flex items-center gap-2 text-xs px-5 py-2 rounded-xl bg-[#3590E3] text-white hover:bg-[#2a7fd4] disabled:opacity-40 transition-all font-medium">{isEdit ? <IoPencilOutline size={13} /> : <IoAddOutline size={13} />}{isEdit ? "Enregistrer" : "Créer le ticket"}</button>
        </div>
      </div>
    </div>
  );
...
function TicketDetailPanel({ ticket, onClose, onEdit, onDelete }) {
  const priorityCfg = { high: { cls: "bg-red-400/10 text-red-400 border-red-400/25", label: "Haute" }, medium: { cls: "bg-yellow-400/10 text-yellow-400 border-yellow-400/25", label: "Moyenne" }, low: { cls: "bg-white/5 text-white/35 border-white/10", label: "Basse" }, urgent: { cls: "bg-red-600/20 text-red-300 border-red-600/30", label: "Urgente" } }[ticket.priority] || { cls: "bg-white/5 text-white/35 border-white/10", label: ticket.priority };
  const statusCfg = { open: { cls: "bg-[#3590E3]/10 text-[#3590E3]", label: "Ouvert", Icon: IoAlertCircleOutline }, pending: { cls: "bg-yellow-400/10 text-yellow-400", label: "En attente", Icon: IoTimeOutline }, "in-progress": { cls: "bg-purple-400/10 text-purple-400", label: "En cours", Icon: IoTimeOutline }, resolved: { cls: "bg-[#BAF09D]/10 text-[#BAF09D]", label: "Résolu", Icon: IoCheckmarkCircleOutline }, closed: { cls: "bg-white/5 text-white/30", label: "Fermé", Icon: IoCheckmarkCircleOutline } }[ticket.status] || { cls: "bg-white/5 text-white/30", label: ticket.status, Icon: IoAlertCircleOutline };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(11,15,26,0.85)", backdropFilter: "blur(8px)" }} onClick={onClose}>
      <div className="w-full max-w-lg bg-[#111827] border border-white/[0.09] rounded-2xl shadow-2xl flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          <div><h3 className="font-medium text-base text-white leading-snug">{ticket.title}</h3>{ticket.description && <p className="text-xs text-white/40 mt-2 leading-relaxed">{ticket.description}</p>}</div>
          <div className="grid grid-cols-2 gap-3">
            {[{ label: "Catégorie", value: ticket.category }, { label: "Priorité", value: priorityCfg.label, highlight: true, cls: priorityCfg.cls }, { label: "Assigné à", value: ticket.assignedTo || "—" }, { label: "Client", value: ticket.client || "—" }, { label: "Créé le", value: ticket.createdAt || "—" }, { label: "Échéance", value: ticket.dueDate || "—" }].map(({ label, value, highlight, cls }) => (
              <div key={label} className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.05]"><p className="text-[0.6rem] text-white/25 mb-1">{label}</p>{highlight ? <span className={`text-[0.65rem] font-semibold px-2 py-0.5 rounded-full border ${cls}`}>{value}</span> : <p className="text-xs text-white/60 font-medium">{value}</p>}</div>
            ))}
          </div>
          {ticket.comments?.length > 0 && (<div><p className="text-[0.65rem] font-medium text-white/25 mb-2">Historique</p>{ticket.comments.map((c, i) => (<div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06] mb-2"><div className="w-6 h-6 rounded-full bg-[#3590E3]/20 border border-[#3590E3]/30 flex items-center justify-center shrink-0 text-[0.55rem] font-unbounded font-bold text-[#3590E3]">{c.author.split(" ").map(w=>w[0]).join("").slice(0,2)}</div><div><div className="flex items-center gap-2 mb-0.5"><span className="text-[0.65rem] font-medium text-white/60">{c.author}</span><span className="text-[0.6rem] text-white/20">{c.time}</span></div><p className="text-xs text-white/50">{c.text}</p></div></div>))}</div>)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <aside className={`flex flex-col h-screen border-r border-white/[0.06] bg-[#0d1220] transition-all duration-300 shrink-0 z-30 ${collapsed ? "w-16" : "w-56"}`}>
      <div className="flex items-center justify-between px-4 py-5 border-b border-white/[0.06]">
        {!collapsed && <span className="font-unbounded font-black text-lg bg-gradient-to-r from-[#3590E3] to-[#BAF09D] bg-clip-text text-transparent tracking-tight">PulsAI</span>}
        <button onClick={() => setCollapsed(v => !v)} className="w-7 h-7 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] flex items-center justify-center text-white/40 hover:text-white transition-all shrink-0">{collapsed ? <IoChevronForwardOutline size={14} /> : <IoChevronBackOutline size={14} />}</button>
      </div>
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        {NAV_ITEMS.map(({ id, label, Icon, badge }) => (
          <button key={id} onClick={() => setActiveView(id)} title={collapsed ? label : ""} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all relative ${activeView === id ? "bg-[#3590E3]/15 text-[#3590E3]" : "text-white/35 hover:text-white/70 hover:bg-white/[0.05]"}`}>
...
function Topbar({ activeView }) {
  ...
  return (
    <header className="h-14 flex items-center justify-between px-5 border-b border-white/[0.06] bg-[#0d1220]/60 backdrop-blur-sm shrink-0">
      <div className="flex items-center gap-3"><h2 className="font-unbounded font-bold text-sm text-white tracking-tight">{labels[activeView]}</h2><span className="text-white/15">·</span><span className="text-xs text-white/25">{new Date().toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long" })}</span></div>
      <div className="flex items-center gap-3">
        <div className="relative hidden md:block"><input type="text" placeholder="Recherche rapide…" className="bg-white/[0.04] border border-white/[0.07] rounded-xl pl-8 pr-4 py-1.5 text-xs text-white placeholder:text-white/20 outline-none focus:border-[#3590E3]/40 transition-all w-48" /><IoSearchOutline size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-white/25" /></div>
        <div className="relative">
          <button onClick={() => setNotifOpen(v => !v)} className="w-8 h-8 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] flex items-center justify-center text-white/40 hover:text-white transition-all relative"><IoNotificationsOutline size={17} /><span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-[#3590E3] rounded-full border border-[#0d1220]" /></button>
          {notifOpen && (
            <div className="absolute right-0 top-10 w-72 bg-[#141922] border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]"><p className="text-xs font-unbounded font-semibold text-white">Notifications</p><button onClick={() => setNotifOpen(false)} className="text-white/30 hover:text-white transition-colors"><IoCloseOutline size={16} /></button></div>
              {NOTIFS.map((n, i) => (<div key={i} className="flex items-start gap-3 px-4 py-3 hover:bg-white/[0.04] cursor-pointer border-b border-white/[0.04] last:border-0 transition-colors"><n.Icon size={15} className={`mt-0.5 shrink-0 ${n.iconColor}`} /><div className="flex-1 min-w-0"><p className="text-xs text-white/70 leading-snug">{n.text}</p><p className="text-[0.6rem] text-white/25 mt-0.5">{n.time}</p></div></div>))}
            </div>
          )}
        </div>
        <Avatar initials={currentUser.avatar} gradient />
        ...
        <div className="xl:col-span-2 bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5">
          ...
          {displayConvs.filter(c => c.status !== "cold").slice(0, 4).map(conv => (
            <div key={conv.id} onClick={() => { setSelectedConv(conv.id); setActiveView("conversations"); }} className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/[0.04] cursor-pointer transition-all group">
              ...
              <div className="text-right shrink-0"><p className="text-[0.65rem] text-white/25">{conv.time}</p>{conv.unread > 0 && <span className="inline-flex mt-1 w-4 h-4 rounded-full bg-[#3590E3] text-white text-[0.55rem] font-bold items-center justify-center">{conv.unread}</span>}</div>
            </div>
          ))}
        </div>
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-5"><HiOutlineChip size={15} className="text-[#3590E3]" /><h2 className="font-unbounded font-semibold text-sm text-white">Pipeline IA</h2></div>
          <div className="space-y-4"><FunnelBar label="Prospection" count={42} max={42} color="#3590E3" /><FunnelBar label="Qualification" count={28} max={42} color="#5aaaf0" /><FunnelBar label="Découverte" count={18} max={42} color="#BAF09D" /><FunnelBar label="Négociation" count={11} max={42} color="#8ef06e" /><FunnelBar label="Conversion" count={6} max={42} color="#28CA41" /></div>
          <div className="mt-5 pt-4 border-t border-white/[0.06] flex justify-between text-xs"><span className="text-white/35">Taux global</span><span className="font-unbounded font-bold text-[#BAF09D]">14.3%</span></div>
        </div>
      </div>
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {AUTOMATIONS.map(({ Icon, title, status, runs }) => (
          <div key={title} className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4 flex items-center gap-4 hover:border-white/[0.12] transition-colors cursor-pointer">
            <div className="w-10 h-10 rounded-xl bg-[#3590E3]/10 flex items-center justify-center shrink-0"><Icon size={18} className="text-[#3590E3]" /></div>
            <div className="flex-1 min-w-0"><p className="text-xs font-medium text-white/75 truncate">{title}</p><p className="text-[0.65rem] text-white/30 mt-0.5">{runs} exécutions</p></div>
            <span className={`text-[0.6rem] font-semibold px-2 py-1 rounded-lg ${status === "active" ? "bg-[#BAF09D]/10 text-[#BAF09D]" : "bg-white/5 text-white/30"}`}>{status === "active" ? "Actif" : "Pausé"}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/[0.06] bg-white/[0.02]">
          <div className="flex items-center gap-3">
            ...
            <div><div className="flex items-center gap-2"><span className="font-medium text-sm text-white">{activeConv.name}</span><ScoreBadge score={activeConv.score} /><span className={`flex items-center gap-1 text-[0.6rem] px-2 py-0.5 rounded-full font-medium ${activeConv.status === "hot" ? "bg-red-400/10 text-red-400" : "bg-yellow-400/10 text-yellow-400"}`}><HiOutlineFire size={10} />{activeConv.status === "hot" ? "Chaud" : "Tiède"}</span></div><p className="text-[0.65rem] text-white/35">{activeConv.email} · {activeConv.stage}</p></div>
          </div>
          <div className="flex items-center gap-2">
            {!handedOff ? (<><div className="flex items-center gap-1.5 bg-[#BAF09D]/10 border border-[#BAF09D]/25 rounded-lg px-3 py-1.5"><TbRobot size={13} className="text-[#BAF09D]" /><span className="text-[0.65rem] text-[#BAF09D] font-medium">IA Pilote</span></div><button onClick={() => setHandedOff(true)} className="flex items-center gap-1.5 text-[0.65rem] px-3 py-1.5 rounded-lg border border-white/10 text-white/40 hover:border-[#3590E3]/40 hover:text-[#3590E3] transition-all"><MdOutlineSupportAgent size={13} /> Prendre la main</button></>) : (<div className="flex items-center gap-1.5 bg-[#3590E3]/10 border border-[#3590E3]/25 rounded-lg px-3 py-1.5"><MdOutlineSupportAgent size={13} className="text-[#3590E3]" /><span className="text-[0.65rem] text-[#3590E3] font-medium">Agent humain</span></div>)}
            <button className="w-8 h-8 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] flex items-center justify-center text-white/40 hover:text-white transition-all"><IoEllipsisHorizontal size={16} /></button>
          </div>
        </div>
          ...
          {messages.map((msg, i) => {
            ...
            return (
              <div key={i} className={`flex items-end gap-2 mb-3 ${isUser ? "justify-end" : "justify-start"}`}>
                {(isAI || isAgent) && <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${isAI ? "bg-[#3590E3]/20 border border-[#3590E3]/30" : "bg-[#BAF09D]/20 border border-[#BAF09D]/30"}`}>{isAI ? <TbRobot size={14} className="text-[#3590E3]" /> : <MdOutlineSupportAgent size={14} className="text-[#BAF09D]" />}</div>}
                <div className={`max-w-[72%] px-4 py-2.5 rounded-2xl text-[0.82rem] leading-relaxed ${isAI ? "bg-white/[0.06] border border-white/[0.08] text-white/80 rounded-bl-sm" : isAgent ? "bg-[#3590E3]/15 border border-[#3590E3]/20 text-white/80 rounded-bl-sm" : "bg-[#BAF09D]/10 border border-[#BAF09D]/15 text-white/80 rounded-br-sm"}`}>{msg.text}<p className="text-[0.6rem] text-white/20 mt-1 text-right">{msg.time}</p></div>
        ...
        {/* ← Bannière paiement : redirige vers PaymentView */}
        {activeConv.score >= 80 && !handedOff && (
          <div className="mx-5 mb-3 p-3 rounded-xl bg-[#BAF09D]/8 border border-[#BAF09D]/20 flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#BAF09D]/15 flex items-center justify-center shrink-0"><HiOutlineCreditCard size={16} className="text-[#BAF09D]" /></div>
            <div className="flex-1 min-w-0"><p className="text-xs font-medium text-[#BAF09D]">Lead prêt à convertir</p><p className="text-[0.65rem] text-white/35">Score {activeConv.score}/100 · Envoyer un lien de paiement KKiaPay</p></div>
            <button onClick={() => setActiveView("payment")} className="shrink-0 flex items-center gap-1.5 text-[0.7rem] font-semibold px-3 py-1.5 rounded-lg bg-[#BAF09D]/20 text-[#BAF09D] hover:bg-[#BAF09D]/30 transition-all">Envoyer <IoSendOutline size={11} /></button>
          </div>
        )}

        <div className="p-4 border-t border-white/[0.06] bg-white/[0.02]">
          <div className="flex items-end gap-3">
            <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} placeholder={handedOff ? "Répondez en tant qu'agent humain…" : "Répondez ou laissez l'IA gérer…"} rows={1} className="flex-1 bg-white/[0.05] border border-white/[0.09] rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/20 outline-none focus:border-[#3590E3]/50 transition-all resize-none leading-snug" style={{ minHeight: "44px", maxHeight: "120px" }} />
            <button onClick={sendMessage} disabled={!input.trim()} className="w-10 h-10 rounded-xl bg-[#3590E3] hover:bg-[#2a7fd4] disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center transition-all shrink-0"><BsArrowUpShort size={22} className="text-white" /></button>
          </div>
      ...
      <div className="hidden xl:flex w-64 shrink-0 border-l border-white/[0.06] flex-col p-4 gap-4 overflow-y-auto">
        <div className="text-center py-3"><Avatar initials={activeConv.avatar} size="lg" gradient /><p className="font-medium text-sm text-white mt-3">{activeConv.name}</p><p className="text-[0.65rem] text-white/35 mt-0.5">{activeConv.email}</p></div>
        <div className="space-y-1">
          {[{ label: "Score IA", value: `${activeConv.score}/100`, highlight: true }, { label: "Étape", value: activeConv.stage }, { label: "Statut", value: activeConv.status === "hot" ? "Chaud" : "Tiède" }, { label: "Contact", value: activeConv.time }].map(({ label, value, highlight }) => (
            <div key={label} className="flex justify-between items-center py-2 border-b border-white/[0.05] last:border-0"><span className="text-[0.7rem] text-white/30">{label}</span><span className={`text-[0.7rem] font-medium ${highlight ? "text-[#BAF09D]" : "text-white/60"}`}>{value}</span></div>
          ))}
        </div>
        <div className="space-y-1">
          ...
          {QUICK_ACTIONS.map(({ Icon, label, action }) => (
            <button key={label} onClick={() => action && setActiveView(action)} className="w-full flex items-center gap-2.5 text-[0.72rem] text-white/45 hover:text-white/75 py-2 px-3 rounded-lg hover:bg-white/[0.05] transition-all">
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8">
      <div className="text-center max-w-sm">
        <div className="w-16 h-16 rounded-2xl bg-[#3590E3]/10 border border-[#3590E3]/20 flex items-center justify-center mx-auto mb-5"><Icon size={28} className="text-[#3590E3]" /></div>
        <h2 className="font-unbounded font-black text-xl text-white mb-2">{title}</h2>
        <p className="text-sm text-white/40 leading-relaxed">{description}</p>
        <div className="mt-6 inline-flex items-center gap-2 text-xs text-[#3590E3] bg-[#3590E3]/10 border border-[#3590E3]/20 px-4 py-2 rounded-full"><span className="w-1.5 h-1.5 rounded-full bg-[#3590E3] animate-pulse" />Vue en cours de développement</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-1.5 bg-white/[0.03] border border-white/[0.06] rounded-xl p-1">{Object.entries(statusLabels).map(([key, label]) => (<button key={key} onClick={() => setFilterStatus(key)} className={`text-[0.65rem] px-2.5 py-1 rounded-lg transition-all font-medium ${filterStatus === key ? "bg-[#3590E3]/20 text-[#3590E3]" : "text-white/30 hover:text-white/60"}`}>{label}</button>))}</div>
        <div className="flex items-center gap-1.5 bg-white/[0.03] border border-white/[0.06] rounded-xl p-1">{Object.entries(priorityLabels).map(([key, label]) => (<button key={key} onClick={() => setFilterPriority(key)} className={`text-[0.65rem] px-2.5 py-1 rounded-lg transition-all font-medium ${filterPriority === key ? "bg-[#3590E3]/20 text-[#3590E3]" : "text-white/30 hover:text-white/60"}`}>{label}</button>))}</div>
      </div>
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5">{filtered.length === 0 ? (<div className="text-center py-12"><IoTicketOutline size={32} className="text-white/10 mx-auto mb-3" /><p className="text-sm text-white/30">Aucun ticket trouvé</p></div>) : (filtered.map(t => <TicketRow key={t.id} ticket={t} onClick={setDetailTicket} onEdit={openEdit} onDelete={id => setDeleteConfirm(id)} />))}</div>
      {modalOpen && <TicketModal ticket={editingTicket} onClose={() => { setModalOpen(false); setEditingTicket(null); }} onSave={handleSave} />}
      {detailTicket && !modalOpen && <TicketDetailPanel ticket={detailTicket} onClose={() => setDetailTicket(null)} onEdit={openEdit} onDelete={id => setDeleteConfirm(id)} />}
      {deleteConfirm && (
